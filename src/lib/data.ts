...
export const getPublicationsByCampaignAndInfluencer = (
  campaignId: string,
  influencerId: string,
) =>
  publications.filter(
    (p) => p.campaignId === campaignId && p.influencerId === influencerId,
  );
...
export const addInfluencersToCampaign = (
  campaignId: string,
...
// 🔁 Actualizar influencer existente
export const updateInfluencer = (updated: Influencer) => {
  influencers = influencers.map((i) =>
    i.id === updated.id ? { ...i, ...updated } : i,
  );
};

// 🗑️ Eliminar influencer (y limpiar relaciones)
export const deleteInfluencer = (id: string) => {
  // quitar de la lista de influencers
  influencers = influencers.filter((i) => i.id !== id);

  // quitar de campañas
  campaigns = campaigns.map((c) => ({
    ...c,
    influencerIds: c.influencerIds.filter((infId) => infId !== id),
  }));

  // quitar sus publicaciones
  publications = publications.filter((p) => p.influencerId !== id);
};
